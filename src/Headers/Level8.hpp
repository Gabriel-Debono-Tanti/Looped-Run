...
void Level8::Init(sf::RenderWindow& window)
{ 
      music.setLoopPoints({sf::milliseconds(0), sf::seconds(38)});
        music.play();
  tmp.Init("Levels/Level8.json", "Textures/tilemap.png", Vector2i(60, 34), Vector2i(32, 32), Vector2i(2048, 2048));
  tiles = tmp.Layer();
...
void Level8::Update(sf::RenderWindow& window)
{ 
   if(goal.finished){
    music.stop();
  }
...
inline void Level8::Events(sf::RenderWindow& window, const optional<Event>& event)
{ 
...
void Level8::Render(sf::RenderWindow& window)
{ 
